...
fun printRealList(xs) = "[" ^ (String.concatWith "," (List.map Real.toString xs)) ^ "]"
fun printRealListList(xs) = "[" ^ (String.concatWith "," (List.map printRealList xs)) ^"]"
fun printRealListListList(xs) = "[" ^ (String.concatWith "," (List.map printRealListList xs)) ^"]"
fun printRealListListListList(xs) = "[" ^ (String.concatWith "," (List.map printRealListListList xs)) ^"]"
fun ugg(f, [], _)= []
  | ugg(f : 'x * 'y -> 'z, (x::xs) : 'x list, ys : 'y list) =
    let
     val temp1 = (List.map (fn y => f(x, y)) ys)
     val temp2 = ugg(f, xs, ys)
    in
     temp1::temp2
    end
