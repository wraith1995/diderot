...
(* QUESTION: should we also check for redefinition of basis functions? *)
	    ...
	    | PT.GD_Type(tyDefBind, {tree=tyName, span=span}, file) =>
	      let
		   ...
		   let
		    val (femType, constants, geometry, cellData) = GFA.validateFemType (cxt,env, tyName) femTyDef file
		    val (methodRefs, methods, newVars, overloads) = Option.getOpt (Option.map (GFA.makeFemMethods (cxt, env, tyName, span) femTyDef file cellData) femType, ([],[],[],[]))
		    ...
		    val argsOption = (case (envI, femType)
					...
					| _ => NONE)
		    val env' = Option.map (GFA.makeDescendentFemTypes (cxt, tyName, span) geometry cellData) argsOption
