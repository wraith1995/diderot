		...
		local
		 ...
		 fun internalCellFuncReplace([seqExp, meshPosExp]) =
		     let
		      ...
		      val facetIdExpr = makePrim'(BV.floor, [AST.E_Slice(seqExp, AST.E_Lit(Literal.intLit 1))], [Ty.T_Real], Ty.T_Int)
						 
		      (*extract seq members*)
		      (*convert 1, be done.*)
		      
		     in
		      AST.E_Apply((nextCellFunc, span), [facetIdExpr, cellExpr, meshExp], Ty.T_Sequence(Ty.T_Int, SOME(Ty.DimConst 2)))
		     end
