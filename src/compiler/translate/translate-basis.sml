...
structure TranslateBasis : sig
   ...
   *)
    ...
    (*       end *)
    fun shapeVarToTensor (Ty.SHAPE shp, Ty.INTERVAL iv) = DstTy.tensorTy shp (if iv = 0
									      then NONE
									      else SOME(iv))
      | shapeVarToTensor _ = raise Fail "expected shape * interval"

    fun shapeVarToTensor' (Ty.SHAPE shp) = DstTy.tensorTy shp NONE
      | shapeVarToTensor' _ = raise Fail "expected shape"
				    
    fun shapeVarToIntList (Ty.SHAPE shp) = shp
      | shapeVarToIntList _ = raise Fail "expected shape"

    fun intervalVarToInt (Ty.INTERVAL iv) = if iv = 0
					    then NONE
					    else SOME(iv)
      | intervalVarToInt _ = raise Fail "expected interval"

    fun intervalToInt i = if i = 0
			  then NONE
			  else SOME(i)
    ...
    fun doLiftedScalar name (y, [iv], xs) = [assignEin(y, name (intervalVarToInt iv), xs)]
    ...
    val tbl : ((IR.var * Ty.meta_arg list * IR.var list) -> IR.assignment list) VTbl.hash_table = let
          ...
          in
            List.app insert [
                ...
                (BV.add_tt,             fn (y, [shp, iv1, iv2], xs) => let
					 val dd1 = shapeVarToIntList shp
					 val iv1' = intervalVarToInt iv1
					 val iv2' = intervalVarToInt iv2
                                          val rator = Mk.addTT(dd1, iv1', iv2')
                                          in
                ...
                (BV.sub_tt,             fn (y, [shp, iv1, iv2], xs) => let
					 val dd1 = shapeVarToIntList shp
					 val iv1' = intervalVarToInt iv1
					 val iv2' = intervalVarToInt iv2
                                          val rator = Mk.subTT(dd1, iv1', iv2')
                                          in
                                            [assignEin(y, rator, xs)]
                                          end),
                ...
                (BV.mul_rr,             fn (y, [iv1, iv2], args) => [assignEin(y, Mk.mulRR(intervalVarToInt iv1, intervalVarToInt iv2), args)]),
                (BV.mul_rt,             fn (y, [shp, iv1, iv2], xs) => let
					 val dd1 = shapeVarToIntList shp
					 val iv1' = intervalVarToInt iv1
					 val iv2' = intervalVarToInt iv2
                                         val rator = Mk.mulRT(dd1, iv1', iv2')
                                          in
                                            [assignEin(y, rator, xs)]
                                          end),
                (BV.mul_tr,             fn (y, [shp, iv1, iv2], [t, r]) => let
					 val dd1 = shapeVarToIntList shp
					 val iv1' = intervalVarToInt iv1
					 val iv2' = intervalVarToInt iv2
                                         val rator = Mk.mulRT (dd1, iv1', iv2')
                ...
                (BV.div_rr,             fn (y, [iv1, iv2], args) =>
					   [assignEin(y, Mk.divRR(intervalVarToInt iv1, intervalVarToInt iv2), args)]),
                (BV.div_tr,             fn (y, [shp, iv1, iv2], xs) => let
					 val dd1 = shapeVarToIntList shp
					 val iv1' = intervalVarToInt iv1
					 val iv2' = intervalVarToInt iv2
                                          val rator = Mk.divTR (dd1, iv1', iv2')
                                          in
                                            [assignEin(y, rator, xs)]
                                          end),
                ...
                (BV.pow_ri,             fn (y, [iv], [f, n]) => case IR.Var.getDef n
                                           of IR.LIT(Literal.Int n) =>
                                                [assignEin(y, Mk.powTI( IntInf.toInt n, intervalVarToInt iv), [f])]
                ...
                (BV.neg_t,              fn (y, [shp, iv1], xs) => let
                                          val ty1 as DstTy.TensorTy (dd1, iv1') = shapeVarToTensor (shp, iv1)
                                          val rator = Mk.negTT (dd1, iv1')
                                          in
                                            [assignEin(y, rator, xs)]
                                          end),
                ...
                (BV.op_norm_t,          fn (y, [sv ,iv], xs) => (case sv
                                           of Ty.SHAPE dd => [assignEin(y, Mk.normT(dd, intervalVarToInt iv), xs)]
                                          (* end case *))),
                (BV.op_norm_f,          fn (y, [ _,Ty.DIM d1, Ty.SHAPE dd1], xs) =>
                                           [assignEin(y, Mk.normF(d1, dd1), xs)]),
                (BV.op_not,             simpleOp Op.BNot),
                (BV.op_cross3_tt,       fn (y, [iv1, iv2], xs) =>
					   [assignEin(y, Mk.cross3TT(intervalVarToInt iv1, intervalVarToInt iv2), xs)]),
                (BV.op_cross2_tt,       fn (y, [iv1, iv2], xs) =>
					   [assignEin(y, Mk.cross2TT(intervalVarToInt iv1, intervalVarToInt iv2), xs)]),
                ...
                (BV.op_outer_tt,        fn (y, [sh1, sh2, _, iv1, iv2], xs) => let
                                          val ty1 as DstTy.TensorTy (dd1, iv1') = shapeVarToTensor (sh1, iv1)
                                          val ty2 as DstTy.TensorTy (dd2, iv2') = shapeVarToTensor (sh2, iv2)
                                          in
                                            [assignEin(y, (Mk.outerTT(dd1, dd2, iv1', iv2')), xs)]
                                          end),
                (BV.op_outer_tf,        fn (y, [_, Ty.DIM d, sh1, Ty.SHAPE dd2, _], xs) => let
                                          val ty1 as DstTy.TensorTy (dd1, _) = shapeVarToTensor' sh1
                                          in
                                            [assignEin(y, Mk.outerTF(d, dd1, dd2), xs)]
                                          end),
                (BV.op_outer_ft,        fn (y, [_, Ty.DIM d, Ty.SHAPE dd1, sh2, _], xs) => let
                                          val ty1 as DstTy.TensorTy (dd2, _) = shapeVarToTensor' sh2
                                          in
                ...
                (BV.op_inner_tt,        fn (y, [sh1, sh2, _, iv1, iv2], xs) => let
                                          val ty1 as DstTy.TensorTy (dd1, iv1') = shapeVarToTensor (sh1, iv1)
                                          val ty2 as DstTy.TensorTy (dd2, iv2') = shapeVarToTensor (sh2, iv2)
                                          in
                                            [assignEin(y, (Mk.innerTT(dd1, dd2, iv1', iv2')), xs)]
                                          end),
                ...
                (BV.op_colon_tt,        fn (y, [sh1, sh2, _, iv1, iv2], xs) => let
                                          val ty1 as DstTy.TensorTy (dd1, iv1') = shapeVarToTensor (sh1, iv1)
                                          val ty2 as DstTy.TensorTy (dd2, iv2') = shapeVarToTensor (sh2, iv2)
                                          in
                                            [assignEin(y, Mk.colonTT(dd1, dd2, iv1', iv2'), xs)]
                                          end),
                ...
                (BV.fn_modulate_tt,     fn (y, [Ty.DIM dd1, Ty.INTERVAL iv1, Ty.INTERVAL iv2], xs) => let
                                          in [assignEin(y, (Mk.modulateTT ([dd1], intervalToInt iv1, intervalToInt iv2)), xs)]
                                          end),
                ...
                (BV.fn_normalize_t,     fn (y, [shp, iv], xs) => let
                                          val DstTy.TensorTy (dd1, iv') = shapeVarToTensor (shp, iv)
                                          in
                                            case (dd1, xs)
                                             of ([], [x]) => raise Fail "fn_normalize_t on scalar"
                                              | (_, [_]) => [assignEin(y, Mk.normalizeTT (dd1, iv', iv'), xs@xs)]
                                            (* end case *)
                                          end),
                (BV.fn_normalize_f,     fn (y, [ _,Ty.DIM d1, Ty.SHAPE dd1], xs) => (case (dd1, xs)
                                          ...
                                          (* end case *))),
                (BV.fn_trace_t,         fn (y, [Ty.DIM d, Ty.INTERVAL iv], xs) =>
                                          [assignEin(y, (Mk.traceT (d, intervalToInt iv)), xs)]),
                (BV.fn_trace_f,         fn (y, [_, Ty.DIM d, Ty.DIM d2, Ty.SHAPE dd], xs) =>
                                          [assignEin(y, Mk.traceF(d, d2, dd), xs)]),
                (BV.fn_transpose_t,     fn (y, [Ty.DIM d1, Ty.DIM d2, Ty.INTERVAL iv], xs) =>
                                          [assignEin(y, (Mk.transposeT ([d1, d2], intervalToInt iv)), xs)]),
...
*)
                (BV.fn_det2_t,          fn (y, [iv], xs) =>
					   [assignEin(y, Mk.det2T (intervalVarToInt iv), xs)]),
                (BV.fn_det3_t,          fn (y, [iv], xs) =>
					   [assignEin(y, Mk.det3T (intervalVarToInt iv), xs)]),
                (BV.fn_det2_f,          fn (y, [_,Ty.DIM d1], xs) =>
                                          [assignEin(y, (Mk.det2F (d1)), xs)]),
                (BV.fn_det3_f,          fn (y, [_,Ty.DIM d1], xs) =>
                                           [assignEin(y, (Mk.det3F (d1)), xs)]),
                (BV.fn_inv1_t,          fn (y, [Ty.INTERVAL iv], xs) =>
					   [assignEin(y, Mk.invR (intervalToInt iv), xs)]),
		(BV.fn_inv2_t,          fn (y, [Ty.INTERVAL iv], xs) =>
					   [assignEin(y, Mk.inv2T (intervalToInt iv), xs)]),
		(BV.fn_inv3_t,          fn (y, [Ty.INTERVAL iv], xs) =>
					   [assignEin(y, Mk.inv3T (intervalToInt iv), xs)]),
