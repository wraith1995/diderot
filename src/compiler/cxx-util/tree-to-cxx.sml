...
structure TreeToCxx : sig
  ...
  (* Translate a TreeIR operator application to a CLang expression *)
    fun trOp (env, rator, cxxArgs) = (case (rator, cxxArgs)
	    ...
	    | (Op.VMin(w, pw), [a, b]) => CL.mkApply(RN.vmin w, [a, b])
	    | (Op.VMax(w, pw), [a, b]) => CL.mkApply(RN.vmax w, [a, b])
	    | (Op.VMaskAndMove(w, pw), [a, b, c])  => CL.mkApply(RN.vmaskandmove w, [a, b, c])
	    | (Op.VAbs(w, pw), [a]) => CL.mkApply(RN.vabs w, [a])
	    | (Op.VAnd(w, pw), [a, b]) => CL.mkBinOp(a, CL.#&& , b)
	    | (Op.VL(ifeq, w, pw), [a, b]) => if ifeq
...
(* FIXME: eventually we should do some kind of liveness analysis to enable in situ operations *)
		...
		of Ty.FemData(FemData.Mesh(_)) => (case ty'
						    of Ty.FemData(FT.MeshCell(_)) =>  CL.E_Grp(CL.E_Select(a, "mesh"))
