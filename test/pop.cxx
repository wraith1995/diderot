...
/*! \file pop.cxx
 *
 * Generated from test/pop.diderot.
 *
 * Command: bin/diderotc --exec test/pop.diderot
 * Version: master:2016-07-29
 */
/*---------- end cxx-head.in ----------*/

#define DIDEROT_STRAND_HAS_CONSTR
#define DIDEROT_NO_GLOBALS
...
static std::string ProgramName = "pop";
...
static diderot::strand_status f_update (world *wrld, f_strand *self);
...
struct strand_array {
  ...
  // invoke strand's update method
    ...
    {
        return f_update(wrld, this->strand(ix));
    }
...
static std::string OutPrefix_r = "r.nrrd";
static std::string OutPrefix_g = "g.nrrd";
static void register_outputs (diderot::options *opts)
{
    opts->add("o-r,output-r", "specify output file for r", &OutPrefix_r, true);
    opts->add("o-g,output-g", "specify output file for g", &OutPrefix_g, true);
}
static void f_init (f_strand *self, int32_t p_i_0)
{
    float l__t_1 = static_cast<float>(p_i_0);
    vec5 v_2 = vcons5(l__t_1, l__t_1, l__t_1, l__t_1, l__t_1);
    float l_vdot_3 = vdot5(v_2, v_2);
    self->sv_r = std::sqrt(l_vdot_3 + l_vdot_3 + l_vdot_3 + l_vdot_3 + l_vdot_3);
    self->sv_g = std::sqrt(l_vdot_3);
}
static diderot::strand_status f_update (world *wrld, f_strand *self)
{
    wrld->print() << self->sv_r << " " << self->sv_g << "\n" << std::flush;
    return diderot::kStabilize;
}
bool output_get_r (world *wrld, Nrrd *nData)
{
    ...
    for (auto ix = wrld->_strands.begin_alive(); ix != wrld->_strands.end_alive(); ix = wrld->_strands.next_alive(ix)) {
        memcpy(cp, &wrld->_strands.strand(ix)->sv_r, 1 * sizeof(float));
        cp += 1 * sizeof(float);
    }
    nData->axis[0].kind = nrrdKindSpace;
    return false;
}
bool output_get_g (world *wrld, Nrrd *nData)
{
    // Compute sizes of nrrd file
    size_t sizes[1];
    sizes[0] = wrld->_size[0];
    ...
    char *cp = reinterpret_cast<char *>(nData->data);
    for (auto ix = wrld->_strands.begin_alive(); ix != wrld->_strands.end_alive(); ix = wrld->_strands.next_alive(ix)) {
        memcpy(cp, &wrld->_strands.strand(ix)->sv_g, 1 * sizeof(float));
        cp += 1 * sizeof(float);
    }
    nData->axis[0].kind = nrrdKindSpace;
    return false;
...
static bool write_output (world *wrld)
{
    Nrrd *nData;
    nData = nrrdNew();
    if (output_get_r(wrld, nData)) {
        wrld->error("Error getting nrrd data for \'r\'");
        return true;
    }
    else if (nrrd_save_helper(OutPrefix_r, nData)) {
        return true;
    }
    nrrdNuke(nData);
    nData = nrrdNew();
    if (output_get_g(wrld, nData)) {
        wrld->error("Error getting nrrd data for \'g\'");
        return true;
    }
    else if (nrrd_save_helper(OutPrefix_g, nData)) {
        return true;
    }
    nrrdNuke(nData);
    return false;
...
bool world::create_strands ()
{
    ...
    uint32_t ix = 0;
    for (int i_i_4 = lo_0; i_i_4 <= hi_1; i_i_4++) {
        f_init(this->_strands.strand(ix), i_i_4);
        ++ix;
    }
    this->swap_state();
