...
/*! \file ugg.cxx
 *
 * Generated from ugg.diderot.
 *
 * Command: ../bin/diderotc --exec ugg.diderot
...
static std::string ProgramName = "ugg";
...
float fn_f (world *wrld, float p_g_0, int32_t p_j_1)
{
    float l_a_2;
    l_a_2 = p_g_0 * 0.2e1f;
    for (int32_t i_i_3 = 0; i_i_3 <= p_j_1; ++i_i_3) {
        float l_a_4;
        wrld->print() << l_a_2 << std::flush;
        if (static_cast<float>(i_i_3) > p_g_0 * 0.5e1f) {
            return 0.e0f;
        }
        else {
            l_a_4 = l_a_2 + p_g_0 * 0.2e0f;
        }
        l_a_2 = l_a_4;
    }
    return l_a_2;
}
static void gg_init (gg_strand *self, int32_t p_i_5)
{
    self->sv_result = 0.1e1f * static_cast<float>(p_i_5);
    self->sv_i = p_i_5;
}
static diderot::strand_status gg_update (world *wrld, gg_strand *self)
{
    wrld->print() << fn_f(wrld, self->sv_result, self->sv_i * 10) << "\n" << std::flush;
    self->sv_result = fn_f(wrld, self->sv_result, self->sv_i * 10);
...
bool world::create_strands ()
{
    ...
    uint32_t ix = 0;
    for (int i_j_6 = lo_0; i_j_6 <= hi_1; i_j_6++) {
        gg_init(this->_strands.strand(ix), i_j_6);
        ++ix;
    }
    this->swap_state();
